...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage contract-design --single
   ```
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage contract-design --result completed
   ```
